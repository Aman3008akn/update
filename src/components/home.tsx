...
export default function Home() {
  ...
  let featuredProducts = getTimeBasedFeaturedProducts(products.filter(p => p.featured), 4);
  let newArrivals = getTimeBasedFeaturedProducts(products.filter(p => p.badges?.includes('new')), 4);
  let bestSellers = getTimeBasedFeaturedProducts(products.filter(p => p.badges?.includes('bestseller')), 4);

  // Fallback: If store has products but none match the tags, show normal products
  if (products.length > 0) {
    if (featuredProducts.length === 0) featuredProducts = products.slice(0, 4);
    if (newArrivals.length === 0) newArrivals = products.slice(0, 4);
