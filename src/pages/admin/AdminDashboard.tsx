...
import { getAllAdminOrders, getAllAdminProducts, deleteAdminProduct, updateAdminOrderStatus } from '@/lib/shopify';
import { mapAdminProductToProduct, mapAdminOrderToOrder } from '@/lib/productMapper';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const shopifyOrders = await getAllAdminOrders(50);
        const mappedOrders = shopifyOrders.map(mapAdminOrderToOrder);

        // Calculate metrics
        const total = mappedOrders.reduce((sum, order) => sum + (order.total_amount || 0), 0);
        setTotalSales(total);
        ...
        // Customers are now managed in Shopify. 
        // Storefront API doesn't allow fetching all customers.
        setActiveUsers(0);

        // Fetch products from Shopify Admin API for low stock
        const shopifyProducts = await getAllAdminProducts(250);
        const lowStock = shopifyProducts.filter(p => (p.total_inventory || 0) < 10).length;
              ...
              {recentOrders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${order.status === 'fulfilled'
...
function ProductsManagement() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const adminProducts = await getAllAdminProducts(250);
      const mapped = adminProducts.map(mapAdminProductToProduct);
      setProducts(mapped);
    } catch (err: any) {
  ...
  const handleAddProduct = async (_productData: any) => {
    setNotification({ message: 'Please add products through your Shopify Admin panel. Products are managed via Shopify.', type: 'info' });
    setTimeout(() => setNotification(null), 5000);
  };

  const handleUpdateProduct = async (_productData: any) => {
    setNotification({ message: 'Please update products through your Shopify Admin panel. Products are managed via Shopify.', type: 'info' });
    setTimeout(() => setNotification(null), 5000);
  };

  const handleDeleteProduct = async (productId: string) => {
    try {
      const success = await deleteAdminProduct(Number(productId));
      if (!success) throw new Error('Failed to delete product via Shopify Admin API');
...
function OrdersManagement() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const adminOrders = await getAllAdminOrders(250);
      const mapped = adminOrders.map(mapAdminOrderToOrder);
      setOrders(mapped);
    } catch (err: any) {
  ...
  const handleUpdateOrderStatus = async (orderData: any) => {
    try {
      const success = await updateAdminOrderStatus(orderData.id, orderData.status);
      if (!success) throw new Error('Failed to update order via Shopify Admin API');
...
function AnalyticsDashboard() {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const adminOrders = await getAllAdminOrders(250);
      const salesRecords = adminOrders.map(o => ({
