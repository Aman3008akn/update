...
// ── FilterPanel (extracted as standalone component) ──
...
export default function CatalogPage() {
    ...
    const loadProducts = async () => {
      try {
        ...
        if (category && category !== 'all') {
          // Try fetching from Shopify collection first
          fetchedProducts = await fetchProductsByCategory(category);
          // If no collection found, fetch all and filter client-side
  ...
  const allSizes = useMemo(() => Array.from(new Set(products.flatMap(p => p.size || []))), [products]);
  const allMaterials = useMemo(() => Array.from(new Set(products.flatMap(p => p.material || []))), [products]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [category, sortBy, priceRange, minRating, selectedSizes, selectedMaterials, selectedBrands, products]);
