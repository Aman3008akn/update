...
import { fetchProductByHandle, fetchProductById } from '@/data/products';
...
export default function ProductPage() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      setLoading(true);
      try {
        // Try fetching by handle first (most common case)
        let fetchedProduct = await fetchProductByHandle(id);
        // If not found, try by ID
        if (!fetchedProduct) {
          fetchedProduct = await fetchProductById(id);
        }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#2C3E50]">Loading product...</p>
        </div>
      </div>
    );
  }
